    ...
    if(err){ res.status(500).json('Sorry, something went wrong on our end while looking for that post');}
      ...
      post.comments.forEach(function(comment){
        if (comment._id.toString() === req.params.comment){
          ...
          if(comment.text !== req.body.text && req.user._id.toString() === comment.author._id.toString()){
            comment.text = req.body.text;
            comment.likes += parseInt(req.body.likes);
            comment.save();
            post.save();
            res.status(200).json(comment);
          } else if (comment.text !== req.body.text && req.user._id.toString() !== comment.author._id.toString()) {
            res.status(401).json("Unauthorized");
          } else {
            comment.likes += parseInt(req.body.likes);
            comment.save();
